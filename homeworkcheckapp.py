...
class TaskManagerApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # 2. ปุ่มเพิ่มงาน (+)
        add_btn = tk.Button(self, text="เพิ่มงาน", font=("Tahoma", 16, "bold"), bg="#ff7f36", fg="white",
                            activebackground="#e66c25", activeforeground="white", bd=0, relief="flat",
                            width=7, height=1, command=self.open_add_dialog, cursor="hand2")
        add_btn.pack(pady=5)
    ...
    def refresh_task_list(self):
        ...
        for idx, task in enumerate(self.tasks):
            ...
            card_bg = "#d9381e" if is_urgent else "#8de4ff"
    ...
    def open_add_dialog(self):
        ...
        dialog.geometry("340x570")
        dialog.configure(bg="#ffffff")
        dialog.grab_set()

        tk.Label(dialog, text="รายละเอียดงาน", font=("Tahoma", 14, "bold"),
        ...
        # ชื่องาน/รายละเอียด (จำกัดตัวอักษรไม่เกิน 50 ตัวอักษร)
        tk.Label(dialog, text="ชื่องาน / รายละเอียด (ไม่เกิน 30 ตัวอักษร):", font=("Tahoma", 9, "bold"), bg="#ffffff").pack(anchor="w", padx=25)
        
        def limit_title(*args):
            value = title_var.get()
            if len(value) > 30:
                title_var.set(value[:30])

        title_var = tk.StringVar()
        title_var.trace_add("write", limit_title)

        title_entry = tk.Entry(dialog, textvariable=title_var, font=("Tahoma", 10))
        title_entry.pack(fill="x", padx=25, pady=(2, 10))

        # เลือกวิชา
        tk.Label(dialog, text="เลือกวิชา", font=("Tahoma", 11, "bold"), bg="#ffffff").pack(pady=(5, 5))

        selected_subject = tk.StringVar(value="คณิตศาสตร์")
        grid_frame = tk.Frame(dialog, bg="#ffffff")
        grid_frame.pack(padx=20, pady=5)

        # ช่องกรอกชื่อวิชาเพิ่มเติมเมื่อเลือก "อื่นๆ"
        other_entry = tk.Entry(dialog, font=("Tahoma", 10))

        def on_subject_change():
            if selected_subject.get() == "อื่นๆ":
                other_entry.pack(fill="x", padx=25, pady=(2, 10))
            else:
                other_entry.pack_forget()

        subjects = ["คณิตศาสตร์", "ภาษาอังกฤษ", "ภาษาไทย", "คอมพิวเตอร์", "สังคมศึกษา", "ประวัติศาสตร์", "ฟิสิกส์", "เคมี", "ชีววิทยา", "อื่นๆ"]
        for idx, subj in enumerate(subjects):
            r, c = divmod(idx, 2)
            btn = tk.Radiobutton(grid_frame, text=subj, value=subj, variable=selected_subject,
                                 indicatoron=0, width=12, height=2, bg="#f3ab7c", selectcolor="#ff5e00",
        ...
        def save_task():
            ...
            # ตรวจสอบวิชาที่เลือก
            if selected_subject.get() == "อื่นๆ":
                custom_subj = other_entry.get().strip()
                final_subject = custom_subj if custom_subj else "อื่นๆ"
            else:
                final_subject = selected_subject.get()
            ...
            try:
                ...
                new_task = {
                    ...
                    "details": f"งานวิชา {final_subject}",
                    "expanded": False
                }
